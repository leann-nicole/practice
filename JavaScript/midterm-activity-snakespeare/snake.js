...
let swipeArea = document.querySelector("#gameBoard");

let gridRows = 10
let gridCols = 10
const keycodes = [37,38, 39, 40] // 37 left, 38 up, 39 right, 40 down
...
function moveTouch(e){ // for touch screens
    e.preventDefault();
    if (drawn){ // ignore direction changes while previous direction change not drawn
        ...
        if (Math.abs(diffX) > Math.abs(diffY)){ //swiped horizontally
            if (diffX > 0) nextDir = 37
            else nextDir = 39
        }
        else{
            if (diffY > 0) nextDir = 38
            else nextDir = 40
        }

        initialX = null; //clear for next swipe
        initialY = null;

        if (Math.abs(direction - nextDir) != 2){
            drawn = false // set to false, turns true after completion of drawSnake() function
            console.log(Math.abs(direction - nextDir))
            direction = nextDir
        }
    }
}
...
function changeDirection(e){ // for keyboard // I don't know how to prevent default here, browser still scrolls down
    if (drawn){ // ignore direction changes while previous direction change not drawn
